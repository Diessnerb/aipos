...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('🔧 PIN ingredients fetch request:', { 
    ...
    // Fetch ingredients
    const { data: ingredients, error: ingredientsError } = await supabaseServiceRole
      .from('ingredients')
      .select('*')
    ...
    console.log(`🥕 Successfully fetched ${ingredients?.length || 0} ingredients`)
    ...
    console.error('🚨 PIN ingredients fetch error:', error)
    return new Response(

...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log('🔧 PIN reservations fetch request:', { 
    ...
    // Fetch reservations using service role (bypasses RLS)
    const { data: reservations, error: reservationsError } = await supabaseServiceRole
      .from('reservations')
      .select(`
        id,
        customer_name,
    ...
    console.log(`📅 Successfully fetched ${reservations?.length || 0} reservations`)

    return new Response(
      JSON.stringify({ 
        success: true, 
        reservations: reservations || [],
        data: reservations || [], // Dual key for backward compatibility
    ...
    console.error('🚨 PIN reservations fetch error:', error)
    return new Response(

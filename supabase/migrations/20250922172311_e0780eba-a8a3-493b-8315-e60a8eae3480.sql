-- First drop the existing function if it exists, then recreate with proper structure
DROP FUNCTION IF EXISTS public.select_contiguous_group_tables(uuid,integer,uuid);
...
BEGIN
  ...
  FOR group_rec IN (
    ...
    JOIN public.tables t ON t.id = tgm.table_id  -- FIX: Use proper table_id join instead of table_number
...
-- Create a helper function to select contiguous tables within groups

-- Update check_comprehensive_availability function to enforce table limits and use natural language
DROP FUNCTION IF EXISTS public.check_comprehensive_availability(uuid,date,time without time zone,integer,integer);
...
DECLARE
  ...
  total_current_tables integer := 0;
  ...
  max_tables_allowed integer := 3;
  max_guest_capacity integer := 18;
BEGIN
  ...
  -- Calculate current guests and tables at the requested time
  SELECT 
    COALESCE(SUM(r.party_size), 0),
    COALESCE(
      SUM(
        CASE 
          WHEN r.table_numbers IS NOT NULL THEN array_length(r.table_numbers, 1)
          WHEN r.table_number IS NOT NULL THEN 1
          ELSE 0
        END
      ), 0
    )
  INTO total_current_guests, total_current_tables
  FROM reservations r
  ...
  -- Determine if booking is available (must satisfy BOTH guest capacity AND table limit)
  is_available := (available_capacity >= p_party_size) AND (total_current_tables < max_tables_allowed);
  ...
  IF NOT is_available THEN
    ...
    FOR i IN 1..6 LOOP
      ...
      -- Check both guest and table availability at this time
      SELECT 
        COALESCE(SUM(r.party_size), 0),
        COALESCE(
          SUM(
      ...
      -- Check if both guest capacity and table limit allow this booking
      IF ((total_capacity - temp_guests) >= p_party_size) AND (temp_tables < max_tables_allowed) THEN
  ...
  ELSE
    IF array_length(suggestion_times, 1) > 0 THEN
      final_message := format('I''m sorry, but we''re fully booked at %s on %s for %s people. However, I have some great alternatives available. How about %s instead? These times have space for your party.',
        to_char(p_time, 'HH12:MI AM'),
        to_char(p_date, 'Day, Month DD'),
        p_party_size,
    ...
    ELSE
      final_message := format('I''m sorry, but we''re fully booked around %s on %s for a party of %s. Would you like to try a different date, or should I check our availability for tomorrow?',
        to_char(p_time, 'HH12:MI AM'),
        to_char(p_date, 'Day, Month DD'),
        p_party_size
      );
  ...
  -- Build current load info (including table usage)
  current_load_info := json_build_object(
    'total_capacity', total_capacity,
    'current_guests', total_current_guests,
    'available_capacity', available_capacity,
    'current_tables_in_use', total_current_tables,
    'max_tables_allowed', max_tables_allowed,
    'tables_available', max_tables_allowed - total_current_tables,
  ...
  business_rules := json_build_object(
    ...
    'max_tables_at_once', max_tables_allowed,

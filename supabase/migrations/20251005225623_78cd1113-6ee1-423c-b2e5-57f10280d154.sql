...
-- Fix PIN authentication to use pgcrypto.digest with proper bytea conversion
-- and qualify the function to avoid search_path issues
...
BEGIN
  ...
  -- Hash the PIN input using SHA-256 with explicit schema/function call
  hashed_pin := encode(extensions.digest(convert_to(pin_input, 'UTF8'), 'sha256'), 'hex');

-- Fix change_team_member_pin to properly verify owner PIN
...
BEGIN
  ...
  -- Verify owner PIN against both hash types
  v_sha256_hash := encode(digest(p_owner_pin, 'sha256'), 'hex');
  ...
  -- Hash new PIN with SHA-256
  v_new_pin_sha256 := encode(digest(p_new_pin, 'sha256'), 'hex');

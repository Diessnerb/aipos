-- Create secure RPC functions for owner PIN management
...
-- Create secure RPC function for updating owner PIN with current PIN verification
...
-- One-time migration to hash existing plaintext owner PINs
DO $$
DECLARE
  rec record;
  v_hashed_pin text;
BEGIN
  -- Only hash PINs that are exactly 4 digits (plaintext) and not already hashed
  FOR rec IN 
    SELECT id, owner_pin 
    FROM public.companies 
    WHERE owner_pin IS NOT NULL 
      AND owner_pin ~ '^[0-9]{4}$'  -- Only 4-digit plaintext PINs
      AND LENGTH(owner_pin) = 4      -- Extra safety check
  LOOP
    -- Hash the plaintext PIN
    v_hashed_pin := md5(rec.owner_pin || 'pin_salt_2025');
    ...
    RAISE LOG 'Hashed owner PIN for company: %', rec.id;
  END LOOP;
  
  RAISE LOG 'Owner PIN migration completed';
END;
$$;
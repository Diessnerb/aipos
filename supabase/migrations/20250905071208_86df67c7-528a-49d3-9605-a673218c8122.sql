-- Bulletproof Table Conflict Prevention - Simplified Approach

-- 1. Clean up existing conflicts first
...
SET status = 'cancelled',
    notes = COALESCE(notes || ' | ', '') || 'Auto-cancelled: Duplicate booking resolved by system'
WHERE id IN (
  SELECT unnest(reservation_ids[2:]) 
  FROM duplicate_reservations
);

-- 2. Create unique constraint for single table reservations only
...
-- 5. Enhanced trigger function to prevent ALL conflicts (including multi-table)
...
BEGIN
  ...
  BEGIN
    ...
    IF conflict_found THEN
      RAISE EXCEPTION 'CONFLICT_DETECTED: Tables % are already booked at % on %. Double booking physically prevented.',
...
-- Create the trigger (bulletproof database-level protection)
...
BEGIN
  RETURN QUERY
  -- Scan for single table conflicts
  SELECT 
    'single_table_conflict'::text as conflict_type,
    r.table_number,
  ...
  -- Scan for multi-table conflicts (manual check since we can't use unique index)
  SELECT 
    'multi_table_conflict'::text as conflict_type,
    (SELECT unnest(r1.table_numbers) LIMIT 1) as table_number,
    r1.date as conflict_date,
    r1.time as conflict_time,
    2::bigint as reservation_count,
    jsonb_build_array(
      jsonb_build_object('id', r1.id, 'customer_name', r1.customer_name),
      jsonb_build_object('id', r2.id, 'customer_name', r2.customer_name)
    ) as details
  FROM public.reservations r1, public.reservations r2
  WHERE r1.company_id = p_company_id
    AND r2.company_id = p_company_id
    AND r1.id != r2.id
    AND r1.date = r2.date
    AND r1.time = r2.time
    AND r1.date >= CURRENT_DATE
    AND r1.status NOT IN ('cancelled', 'no-show')
    AND r2.status NOT IN ('cancelled', 'no-show')
    AND (
      (r1.table_numbers IS NOT NULL AND r2.table_number = ANY(r1.table_numbers))
      OR (r2.table_numbers IS NOT NULL AND r1.table_number = ANY(r2.table_numbers))
      OR (r1.table_numbers IS NOT NULL AND r2.table_numbers IS NOT NULL 
          AND r1.table_numbers && r2.table_numbers)
    )

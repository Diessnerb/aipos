...
BEGIN
  ...
  ELSE
    ...
    FOR v_slot_offset IN 1..12 LOOP -- Check up to 12 slots (3 hours in each direction)
      -- Try forward direction first (+15, +30, +45, etc.)
      v_direction := 1;
      ...
      IF v_check_minutes >= 600 AND v_check_minutes < 1320 THEN
      ...
      -- Then try backward direction (-15, -30, -45, etc.)
      v_direction := -1;
      v_check_minutes := v_requested_minutes + (v_direction * v_slot_offset * 15);
      ...
      -- Exit if we have enough suggestions
      IF v_suggestion_count >= v_max_suggestions THEN
        EXIT;
      END IF;
    END LOOP;

-- Update secure_table_update to handle service_status
CREATE OR REPLACE FUNCTION secure_table_update(
  p_table_id uuid,
...
-- Update secure_table_insert to handle service_status and set default
CREATE OR REPLACE FUNCTION secure_table_insert(
...
BEGIN
  ...
  INSERT INTO tables (
    ...
    p_accessibility_friendly, p_description, p_is_active, p_can_combine, p_max_combine_size,
    ...
    p_is_business_friendly, p_service_status
  )
...
-- Backfill existing tables with NULL service_status to 'available'
UPDATE tables 

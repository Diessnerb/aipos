...
DROP TRIGGER IF EXISTS trigger_handle_new_customer ON public.reservations CASCADE;
DROP TRIGGER IF EXISTS insert_customer_on_reservation ON public.reservations CASCADE;
...
CREATE TRIGGER trigger_handle_reservation_customer AFTER INSERT ON public.reservations
    FOR EACH ROW
    EXECUTE FUNCTION public.handle_reservation_customer();

-- Create trigger for visit counting on status update to completed
DROP TRIGGER IF EXISTS trigger_handle_completed_reservation ON public.reservations;
CREATE TRIGGER trigger_handle_completed_reservation AFTER UPDATE ON public.reservations
    FOR EACH ROW

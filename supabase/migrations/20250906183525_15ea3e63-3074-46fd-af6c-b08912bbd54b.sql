-- Update resequence_table_group to accept optional company_id and validate ownership
DROP FUNCTION IF EXISTS public.resequence_table_group(p_group_id uuid, p_company_id uuid) CASCADE;
CREATE OR REPLACE FUNCTION public.resequence_table_group(p_group_id uuid, p_company_id uuid DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_company_id IS NOT NULL THEN
    IF group_company_id != p_company_id THEN
      RAISE EXCEPTION 'Access denied: Group does not belong to specified company';
    END IF;
  ELSE
  ...
  -- Resequence the tables in the group
  UPDATE table_group_memberships tgm
  SET priority_order = subq.new_order
  FROM (
    SELECT 
      tgm2.table_id,
      ROW_NUMBER() OVER (ORDER BY tgm2.priority_order) - 1 as new_order
    FROM table_group_memberships tgm2
    WHERE tgm2.group_id = p_group_id
  ) subq
  WHERE tgm.group_id = p_group_id 
    AND tgm.table_id = subq.table_id;
END;
$function$;

-- Update move_table_in_group to accept optional company_id and validate ownership
DROP FUNCTION IF EXISTS public.move_table_in_group(p_table_id uuid, p_group_id uuid, p_direction text, p_company_id uuid) CASCADE;

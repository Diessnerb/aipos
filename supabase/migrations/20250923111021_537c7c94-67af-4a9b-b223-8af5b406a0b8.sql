-- Clean up overloaded RPCs and create canonical versions
-- 1) Drop ALL versions of secure_table_update and secure_table_insert dynamically
DO $$
DECLARE
  rec RECORD;
BEGIN
  FOR rec IN 
    ...
    WHERE n.nspname = 'public'
      AND p.proname IN ('secure_table_update','secure_table_insert')
  LOOP
    EXECUTE format('DROP FUNCTION IF EXISTS public.%I(%s);', rec.proname, rec.args);
  END LOOP;
END $$;

-- 2) Recreate canonical secure_table_insert with clear, unambiguous signature
DROP FUNCTION IF EXISTS public.secure_table_insert(p_table_number integer, p_seats integer, p_table_name text, p_location text, p_status text, p_shape text, p_type text, p_accessibility_friendly boolean, p_description text, p_is_active boolean, p_can_combine boolean, p_max_combine_size integer, p_group_priority integer, p_features jsonb, p_vip_status boolean, p_window_seating boolean, p_privacy_level text, p_ambiance text, p_is_high_top boolean, p_is_main_dining boolean, p_is_outdoor boolean, p_is_quiet_area boolean, p_is_family_friendly boolean, p_is_business_friendly boolean, p_service_status text, p_floor_level integer, p_x_position double precision, p_y_position double precision, p_rotation double precision) CASCADE;
...
BEGIN
  ...
  INSERT INTO public.tables (
    ...
    is_business_friendly, service_status, floor_level,
    floor_plan_x, floor_plan_y, floor_plan_rotation,
    x_position, y_position, rotation
  ) VALUES (
    v_company_id, p_table_number, p_table_name, p_seats, p_location, p_status, p_shape, p_type,
    p_accessibility_friendly, p_description, COALESCE(p_is_active, true), p_can_combine, p_max_combine_size,
    p_group_priority, p_features, p_vip_status, p_window_seating, p_privacy_level, p_ambiance,
    p_is_high_top, p_is_main_dining, p_is_outdoor, p_is_quiet_area, p_is_family_friendly,
    p_is_business_friendly, p_service_status, p_floor_level,
    p_x_position, p_y_position, p_rotation,
    p_x_position, p_y_position, p_rotation
  ) RETURNING id INTO v_new_id;

  RETURN json_build_object('success', true, 'message', 'Table created', 'id', v_new_id);
EXCEPTION WHEN OTHERS THEN
  RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$;

-- 3) Recreate canonical secure_table_update with clear, unambiguous signature
DROP FUNCTION IF EXISTS public.secure_table_update(p_table_id uuid, p_table_number integer, p_table_name text, p_seats integer, p_location text, p_status text, p_shape text, p_type text, p_accessibility_friendly boolean, p_description text, p_is_active boolean, p_can_combine boolean, p_max_combine_size integer, p_group_priority integer, p_features jsonb, p_vip_status boolean, p_window_seating boolean, p_privacy_level text, p_ambiance text, p_is_high_top boolean, p_is_main_dining boolean, p_is_outdoor boolean, p_is_quiet_area boolean, p_is_family_friendly boolean, p_is_business_friendly boolean, p_service_status text, p_floor_level integer, p_x_position double precision, p_y_position double precision, p_rotation double precision) CASCADE;
...
BEGIN
  ...
  UPDATE public.tables SET
    ...
    floor_plan_x = COALESCE(p_x_position, floor_plan_x),
    floor_plan_y = COALESCE(p_y_position, floor_plan_y),
    floor_plan_rotation = COALESCE(p_rotation, floor_plan_rotation),
  ...
  RETURN json_build_object('success', true, 'message', 'Table updated');
EXCEPTION WHEN OTHERS THEN
  RETURN json_build_object('success', false, 'error', SQLERRM);
END;
$$;
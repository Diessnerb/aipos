-- Fix the table groups RPC function to use correct joins and return proper data structure
  ...
  FOR group_rec IN (
    ...
    FOREACH table_num IN ARRAY group_rec.tables LOOP
      ...
      ELSE
        ...
        SELECT COALESCE(t.seats, 0) INTO total_free_seats 
        FROM public.tables t 
        WHERE t.table_number = table_num AND t.company_id = p_company_id;
        total_free_seats := total_free_seats + COALESCE(total_free_seats, 0);
      END IF;
    END LOOP;
    ...
    -- Even if not fully available, return if there are free tables that can accommodate
    IF array_length(free_tables_list, 1) > 0 AND total_free_seats >= p_party_size THEN
